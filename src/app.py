...
for i, ll in enumerate(bpt_lines):
    zooms[:,i] = lines_waves[ll]

app = dash_script.dash_plot_spectra(x={'Continuum UMAP axis 1': embeddings_x},
                                    y={'Continuum UMAP axis 2': embeddings_y},
                                    color_code={'H_alpha EW': np.clip(h_alpha_ews, a_min=0, a_max=20)},
                                    spectra=[rest_frame_spectra, fluxes],
                                    spec_colors=['rgba(0,256,256,0.5)', 'rgba(240,128,128,1)'],
                                    spec_names=['Observed Spectrum', 'Masked Continuum'],
                                    wavelength=[wavelength, bin_centers],
                                    kao_lines=False,
                                    zoom={'1': zooms[:,0], '2': zooms[:,1], '3': zooms[:,2], '4': zooms[:,3]},
                                    zoom_windows=[15, 15, 15, 15],
